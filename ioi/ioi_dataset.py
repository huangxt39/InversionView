"""
This is taken from ACDC paper: https://github.com/ArthurConmy/Automatic-Circuit-Discovery/blob/main/acdc/ioi/ioi_dataset.py

It is a very slightly edited version
"""
...
class IOIDataset:
    def __init__(
        ...
        for i in range(N):
            try:
                ...
                self.templates_by_prompt.append(None)
    ...
    def gen_rand_person_prompts(self, seed):
        """
        Return a IOIDataset where the name to flip has been replaced by a random name.
        """

        seed = (seed+345467)%5432
        np.random.seed(seed)

        flipped_prompts = []

        for prompt in tqdm(self.ioi_prompts):
            prompt = prompt.copy()
            t = prompt["text"].split(" ")

            existing_id = []
            for i in range(len(t)):
                if t[i] == prompt["S"] or t[i] == prompt["IO"]:
                    rand_id = np.random.randint(len(NAMES))
                    while rand_id in existing_id:
                        rand_id = np.random.randint(len(NAMES))
                    t[i] = NAMES[rand_id]
                    existing_id.append(rand_id)
